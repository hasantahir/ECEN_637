function Abbas_HW_11_Parabolic_equation_temp_local()
% Hasan Tahir Abbas
% ECEN 637
...
global pe za dx pp a a_e 
global fmax ftemp zz
global p z pat_p f_z
global pattern fz  m
global freq lambda c kappa  theta_max
global beamwidth Length
global epsi_o epsi_r sigma omega index eps_c
global han dNdz zmax Picture
global xx ii choice
% *************************
% *************************
...
global freq lambda c kappa  theta_max
global beamwidth Length
global epsi_o epsi_r sigma omega index epsi_c
global han dNdz zmax Picture
global xx ii
...
ii = -1;
temp = replace(ii); % Store data in temporary array for calculation
temp = fft(temp,N);
pattern = temp*dz;
draw_data();
% save pattern.mat
...
ii = 1;
temp = replace(ii);  % Store data in temporary array
temp = pre_process(temp);  % Take care of marching
temp = N*ifft(temp,N);
post_process(temp); % Take care of index difference in z-direction
fmax = max_fz(temp);
...
% *************************
% *************************
function out = post_process(temp)
...
for i = 1 : N
    ...
    temp(i) = temp(i)* exp(1i* kappa * m(i) * dx/2) * han;
end
out = temp;
end % end function
% *************************
