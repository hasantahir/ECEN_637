function Abbas_HW_11_Parabolic_equation()
% Hasan Tahir Abbas
% ECEN 637
...
%         disp('           Choice = 2                  ');
        for i = 1 : N
            if (z(i) < 0) % Sea water
                ...
                index = 1 + (300 - .0394*z(i))*1e-6; % Standard atmosphere
...
%         fz(k) = exp(1i*pe*z(k))*...
%           ( 1 + ...
%             2 * ( 0.0977020907 ) * cos(2*pi*1.0*zz/Length) ...
%           + 2 * ( 0.045938932 ) * cos(2*pi*2.0*zz/Length) ...
%           + 2.0 * (-0.0567750651) * cos(2*pi*3.0*zz/Length)...
%           + 2.0 * ( 0.0543140899)*cos(2*pi*4.0*zz/Length)...
%           + 2.0 * (-0.0472939433)*cos(2*pi*5.0*zz/Length)...
%           + 2.0 * ( 0.0381149255)*cos(2*pi*6.0*zz/Length)...
%           + 2.0 * (-0.0279769765)*cos(2*pi*7.0*zz/Length)...
%           + 2.0 * ( 0.0177444933)*cos(2*pi*8.0*zz/Length)...
%           + 2.0 * (-0.0081655839)*cos(2*pi*9.0*zz/Length)...
%           )/Length; 
%     else
%         fz(k)=0.0;
%     end
...
ii = -1;
temp = replace(); % Store data in temporary array for calculation
% temp = four1(temp, N, ii); % FFT of Data
temp = fft(temp);
my_fft = temp;
pattern = temp*dz;
draw_data();
...
% temp = four1(temp,N,ii);   % Inverse FFT
temp = ifft(temp);
