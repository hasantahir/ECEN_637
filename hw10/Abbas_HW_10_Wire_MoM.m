...
%% Currents
I_wire = Pocklington_Current(.7*lambda, .001588*lambda); % to match the given figure
...
for j = 1 : 1 % Only one iteration is needed as impedance matrix can be constructed through its Toeplitz property
    for k = 1 : M 
        ...
        fun = @(zp) exp(-1i*K*sqrt(( z(j)+.5*deltaz - zp).^2 + a^2))...
            ./sqrt(( z(j)+.5*deltaz - zp).^2 + a^2); % Make a symbolic Function of z_prime
        % This calculation is based on the reference cited in the code
        % introduction. 
        % Mathematical Modeling of Eq. 4.63 
        zp_upper = z(k) + deltaz; % Upper limit of integration
        dz_upper = z(j)+.5*deltaz - zp_upper; % Represents (z(m) - z(n)) for the upper limit
        zp_lower = z(k); % Lower limit of integration
        dz_lower = z(j)+.5*deltaz - zp_lower; % Represents (z(m) - z(n)) for the limit limit
...
% First Polar Plot
figure(3)
h3 = polar(theta, abs(E_rad));
h3.Color = 'black';
h3.LineWidth = 1.4;
title(['Radiation Pattern of a Wire of length $\lambda$ at f clo=  ',int2str(f/1e6), ' MHz'],'Interpreter','latex')
set(gcf,'Color','white'); % Set background color to white
set (gca,'FontName','times new roman') % Set axes fonts to Times New Roman
% cleanfigure();
