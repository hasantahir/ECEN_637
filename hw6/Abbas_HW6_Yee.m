...
% *************************
% *************************
for k = 1 : 2  % Iteration to implement two rounds of simulation:
    % 1. Free-space
    % 2. PEC box included
    
    define_media(iflaga); % Simulation Structure Definition
...
legend('n=95')
legend('boxoff')
if iflaga == 1  % This part creates a title for the subplot which can not be done by default
    set(gcf,'NextPlot','add');
    axes;
    h = title('Line Plots for E-field with no obstacle');
    set(gca,'Visible','off');
    set(h,'Visible','on');
else
    set(gcf,'NextPlot','add');
    axes;
    h = title('Line Plots for E-field with a PEC box');
    set(gca,'Visible','off');
    set(h,'Visible','on');
end
