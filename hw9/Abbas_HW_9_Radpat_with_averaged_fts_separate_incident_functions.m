function [Ez_nrm, Ezi_nrm, sweep] = Abbas_HW_9_Radpat_with_averaged_fts_separate_incident_functions()
% Hasan Tahir Abbas
% ECEN 637
...
for n = 1 : nt
    ...
    adv_Ezi(); % Compute E-field
    Liao_ABCi(); % Invoke ABC algorithm
    adv_H(); % Compute total H-field
    adv_Hi(); % Compute incident H-field
...
%     sum_field(); % Sums the FT-fields to compute far-field (Eq. 24)
% end
%  polar(phi,abs(Ez_norm-Ezi_norm))
...
% *************************
%
%% Implement LIAO ABC for total field
%
% *************************
% *************************
function Liao_ABC()
...
%
%
%% Implement LIAO ABC for incident field
%
% *************************
% *************************
function Liao_ABCi()
...
%
%
%% Algorithm for Computing total E-field
% *************************
% *************************
function adv_Ez()
...
%% Algorithm for Computing incident E-field
% *************************
% *************************
function adv_Ezi()
% Compute z-component of E-field
...
if (flag_medium == 1)
    for i = 1 : nx
        for j = 1 : ny
            m  = mediaEz(i,j); % Enforce free-space everywhere
            if (i == mxcite)   %% Incident Field Source Excitation
                Es = Source(); % Es is a soft source
                Source_signal(n) = Es;
            else
...
% Space with box Hy-field computation
if (flag_medium == 2)
    for i = 1 : nx
        for j = 1 : ny - 1
            m = mediaHx(i,j);
...
%% Algorithm for Computing incident H-field 
% *************************
% *************************
function adv_Hi()
% Compute z-component of E-field
...
global ftEzi_right ftEzi_top ftEzi_left ftEzi_bottom
global ftHxi_top ftHxi_bottom
global ftHyi_right ftHyi_left
...
for i = 1 : (myct - mycb)    
    ...
    ftHy_right(i) = ftHy_right(i) + dt * (Hy(X,Y+i-1) + Hy(X-1,Y+i-1)) * exp(dft_exp)/2;	%%%!incident field dft
    ftEzi_right(i) = ftEzi_right(i) + dt * Ezi(X,Y+i-1) * exp(dft_exp);	%%%!incident field dft
    ftHyi_right(i) = ftHyi_right(i) + dt * (Hyi(X,Y+i-1) + Hyi(X-1,Y+i-1)) * exp(dft_exp)/2;	%%%!incident field dft
...
for i = 1 : (mxcr - mxcl)
        ...
        ftHx_top(i) = ftHx_top(i) + dt * (Hx(X-i+1,Y) + Hx(X-i+1,Y-1)) * exp(dft_exp)/2;	%%%!incident field dft
        ftEzi_top(i) = ftEzi_top(i) + dt * Ezi(X-i+1,Y) * exp(dft_exp);	%%%!incident field dft
        ftHxi_top(i) = ftHxi_top(i) + dt * (Hxi(X-i+1,Y) + Hxi(X-i+1,Y-1)) * exp(dft_exp)/2;	%%%!incident field dft
...
for i = 1 : (myct - mycb)
        ...
        ftHy_left(i) = ftHy_left(i) + dt * (Hy(X,Y-i+1) + Hy(X-1,Y-i+1)) * exp(dft_exp)/2;	%%%!incident field dft
        ftEzi_left(i) = ftEzi_left(i) + dt * Ezi(X,Y-i+1) * exp(dft_exp);	%%%!incident field dft
        ftHyi_left(i) = ftHyi_left(i) + dt * (Hyi(X,Y-i+1) + Hyi(X-1,Y-i+1)) * exp(dft_exp)/2;	%%%!incident field dft
...
for i = 1 : (mxcr - mxcl)
        ...
        ftHx_bottom(i) = ftHx_bottom(i) + dt * (Hx(X+i-1,Y) + Hx(X+i-1,Y-1)) * exp(dft_exp)/2;	%%%!incident field dft
        ftEzi_bottom(i) = ftEzi_bottom(i) + dt * Ezi(X+i-1,Y) * exp(dft_exp);	%%%!incident field dft
        ftHxi_bottom(i) = ftHxi_bottom(i) + dt * (Hxi(X+i-1,Y) + Hxi(X+i-1,Y-1)) * exp(dft_exp)/2;	%%%!incident field dft
